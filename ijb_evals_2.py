...
def extract_IJB_data_11(data_path, subset, save_path=None, force_reload=False):
    ...
    templates, medias = read_IJB_meta_columns_to_int(media_list_path, columns=[1, 2])
    print("templates: %s, medias: %s, unique templates: %s" % (templates.shape, medias.shape, np.unique(templates).shape))

    print(">>>> Loading pairs...")
    p1, p2, label = read_IJB_meta_columns_to_int(pair_list_path, columns=[0, 1, 2])
    ...
    with open(img_list_path, "r") as f:
        img_records = np.array([ii.strip().split(" ") for ii in f.readlines()])
    ...
    counts, bins = np.histogram(face_scores, bins=9)
    print("face_scores value counts:", dict(zip(bins[:-1], counts)))
...
class IJB_test:
    ...
    def run_model_test_1N(self, npoints=100):
        fars_cal = [10 ** ii for ii in np.arange(-4, 0, 4 / npoints)] + [1]
        fars_show_idx = np.arange(len(fars_cal))[:: npoints // 4]
        ...
        print("g1_templates_feature:", g1_templates_feature.shape)
        print("g2_templates_feature:", g2_templates_feature.shape)
...
def calculate_accuracy(scores, label, threshold=None, fpr_target=None):
    """
    Calculate general accuracy for 1:1 verification.
    
    Args:
        scores: Array of similarity scores for template pairs.
        label: Ground-truth labels (1 for genuine, 0 for impostor).
        threshold: Fixed threshold for classification (optional).
        fpr_target: Target FPR to determine threshold (e.g., 0.001) (optional).
    
    Returns:
        accuracy: General accuracy (TP + TN) / (TP + TN + FP + FN).
        threshold: Threshold used for classification.
    """
    if threshold is None and fpr_target is not None:
        # Compute ROC to find threshold at target FPR
        fpr, tpr, thresholds = roc_curve(label, scores)
        idx = np.argmin(np.abs(fpr - fpr_target))
        threshold = thresholds[idx]
    elif threshold is None:
        # Default: Use EER threshold
        fpr, tpr, thresholds = roc_curve(label, scores)
        fnr = 1 - tpr
        eer_idx = np.argmin(np.abs(fpr - fnr))
        threshold = thresholds[eer_idx]
    
    # Classify pairs based on threshold
    predictions = (scores >= threshold).astype(int)
    accuracy = accuracy_score(label, predictions)
    
    print(f"Accuracy: {accuracy:.6f} at threshold {threshold:.6f}")
    return accuracy, threshold


def plot_roc_and_calculate_tpr(scores, names=None, label=None):
    print("Computing ROC, TPR, and accuracy...")
    score_dict = {}
    for id, score in enumerate(scores):
        ...
        if isinstance(score, str) and score.endswith(".npz"):
            aa = np.load(score)
            score = aa.get("scores", [])
            label = aa.get("label") if label is None and "label" in aa else label
    ...
    fpr_dict, tpr_dict, roc_auc_dict, tpr_result, acc_result = {}, {}, {}, {}, {}
    for name, score in score_dict.items():
        ...
        # Calculate accuracy at EER and FPR=0.001
        acc_eer, thresh_eer = calculate_accuracy(score, label, threshold=None)  # EER
        acc_fpr, thresh_fpr = calculate_accuracy(score, label, fpr_target=0.001)  # FPR=0.001
        acc_result[name] = [acc_eer, acc_fpr]
    
    tpr_result_df = pd.DataFrame(tpr_result, index=x_labels).T
    tpr_result_df["AUC"] = pd.Series(roc_auc_dict)
    acc_result_df = pd.DataFrame(acc_result, index=["Accuracy@EER", "Accuracy@FPR=0.001"]).T
    result_df = pd.concat([tpr_result_df, acc_result_df], axis=1)
    result_df.columns.name = "Methods"
...
def plot_dir_far_cmc_scores(scores, names=None):
    try:
        ...
        for id, score in enumerate(scores):
            ...
            label = "[%s (AUC = %0.2f%%)]" % (name, auc_value * 100)
...
def parse_arguments(argv):
    ...
    parser.add_argument("-P", "--plot_only", nargs="*", type=str, help="Plot saved results, Format 1 2 3 or 1.npy")
    args = parser.parse_args(argv)

    if args.plot_only is not None and len(args.plot_only) != 0:
        from glob2 import glob
        score_files = []
        for ss in args.plot_only:
            score_files.extend(glob(ss.replace(".", "").strip()))
        args.plot_only = score_files
    elif args.model_file is None and args.save_result == default_save_result_name:
        print("Please provide -m MODEL_FILE, see `--help` for usage.")
        exit(1)
    elif args.model_file is not None:
        if args.model_file.endswith((".h5", ".pth", ".pt", ".onnx")):
...
if __name__ == "__main__":
    ...
    if args.plot_only is not None and len(args.plot_only) != 0:
        ...
        save_name = os.path.splitext(os.path.splitext(args.save_result))[0]
        save_items = {}
        ...
        if args.model_file is not None or args.save_embeddings:
